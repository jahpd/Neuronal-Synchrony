$(function() {
    ...
    interacting = false, $window = $(window);
	...
	Sound.ready(function() {
		...
		soundsBuffered();
		socket.emit('user loaded', {user:{hash:myhash}});
		socket.on('user loaded', function(msg){
		    console.log("=> User "+msg.user.hash+" ready to play!")
		});
	    });
	});
    }
	...
	var json = {animation:{hash:hash}, user:{hash:myhash}};
	socket.emit('user trigger', json);
	socket.on('user trigger', function(msg){
	    var animation = animations.map[msg.animation.hash];
	    console.log('=> NEURON: '+msg.animation.hash+' by '+msg.user.hash);
	    if (animation) {
